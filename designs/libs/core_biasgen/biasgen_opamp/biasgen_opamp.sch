v {xschem version=3.4.7 file_version=1.2}
G {}
K {}
V {}
S {}
E {}
L 3 110 -710 110 -670 {}
L 3 110 -670 120 -680 {}
L 3 100 -680 120 -680 {}
L 3 100 -680 110 -670 {}
L 3 350 -710 350 -670 {}
L 3 350 -670 360 -680 {}
L 3 340 -680 360 -680 {}
L 3 340 -680 350 -670 {}
L 3 660 -710 660 -670 {}
L 3 660 -670 670 -680 {}
L 3 650 -680 670 -680 {}
L 3 650 -680 660 -670 {}
B 3 60 -930 660 -770 {fill=false}
B 3 10 -630 730 -550 {fill=false}
B 3 210 -250 710 -170 {fill=false}
B 3 200 -440 490 -360 {fill=false}
A 3 47.43902439024399 -662.80487804878 46.22096014206645 126.416378519886 360 {}
P 4 1 720 -490 {}
T {2u} 100 -740 0 0 0.4 0.4 {}
T {2u} 340 -740 0 0 0.4 0.4 {}
T {8u} 650 -740 0 0 0.4 0.4 {}
T {IDC (typ)=12u} -100 -740 0 0 0.4 0.4 {}
T {Dummies} 80 -980 0 0 0.4 0.4 {}
N 260 -480 260 -430 {lab=vptail}
N 260 -480 350 -480 {lab=vptail}
N 350 -520 350 -480 {lab=vptail}
N 440 -480 440 -430 {lab=vptail}
N 350 -480 440 -480 {lab=vptail}
N 350 -560 350 -520 {lab=vptail}
N 350 -660 350 -620 {lab=VDD}
N 150 -590 310 -590 {lab=vpref}
N 110 -650 110 -620 {lab=VDD}
N 110 -660 110 -650 {lab=VDD}
N 110 -660 350 -660 {lab=VDD}
N 260 -370 260 -240 {lab=vnload}
N 300 -210 400 -210 {lab=vnload}
N 260 -270 320 -270 {lab=vnload}
N 320 -270 320 -210 {lab=vnload}
N 440 -370 440 -240 {lab=vout_1st}
N 260 -180 260 -150 {lab=VSS}
N 260 -150 440 -150 {lab=VSS}
N 440 -180 440 -150 {lab=VSS}
N 250 -200 250 -150 {lab=VSS}
N 440 -210 450 -210 {lab=VSS}
N 450 -210 450 -150 {lab=VSS}
N 440 -150 450 -150 {lab=VSS}
N 250 -150 260 -150 {lab=VSS}
N 250 -210 250 -200 {lab=VSS}
N 250 -210 260 -210 {lab=VSS}
N 540 -210 620 -210 {lab=vout_1st}
N 540 -270 540 -210 {lab=vout_1st}
N 440 -280 540 -280 {lab=vout_1st}
N 540 -280 540 -270 {lab=vout_1st}
N 660 -180 660 -150 {lab=VSS}
N 450 -150 520 -150 {lab=VSS}
N 520 -150 660 -150 {lab=VSS}
N 660 -210 670 -210 {lab=VSS}
N 670 -210 670 -150 {lab=VSS}
N 660 -150 670 -150 {lab=VSS}
N 180 -590 180 -530 {lab=vpref}
N 110 -530 180 -530 {lab=vpref}
N 350 -660 660 -660 {lab=VDD}
N 660 -660 660 -620 {lab=VDD}
N 570 -590 620 -590 {lab=vpref}
N 180 -530 560 -530 {lab=vpref}
N 570 -590 570 -530 {lab=vpref}
N 560 -530 570 -530 {lab=vpref}
N 660 -590 670 -590 {lab=VDD}
N 670 -660 670 -590 {lab=VDD}
N 660 -660 670 -660 {lab=VDD}
N 350 -590 360 -590 {lab=VDD}
N 360 -660 360 -590 {lab=VDD}
N 100 -590 110 -590 {lab=VDD}
N 100 -660 100 -590 {lab=VDD}
N 100 -660 110 -660 {lab=VDD}
N 660 -560 660 -240 {lab=vout}
N 260 -400 300 -400 {lab=VDD}
N 400 -400 440 -400 {lab=VDD}
N 200 -400 220 -400 {lab=vn}
N 480 -400 490 -400 {lab=vp}
N 480 -400 490 -400 {lab=vp}
N 660 -400 800 -400 {lab=vout}
N 80 -660 100 -660 {lab=VDD}
N 220 -150 250 -150 {lab=VSS}
N 540 -360 660 -360 {lab=vout}
N 540 -300 540 -280 {lab=vout_1st}
N 220 -830 230 -830 {lab=VDD}
N 220 -880 220 -830 {lab=VDD}
N 220 -880 230 -880 {lab=VDD}
N 230 -880 230 -860 {lab=VDD}
N 220 -830 220 -800 {lab=VDD}
N 220 -800 230 -800 {lab=VDD}
N 270 -880 270 -830 {lab=VDD}
N 230 -880 270 -880 {lab=VDD}
N 420 -830 430 -830 {lab=VDD}
N 420 -880 420 -830 {lab=VDD}
N 420 -880 430 -880 {lab=VDD}
N 430 -880 430 -860 {lab=VDD}
N 420 -830 420 -800 {lab=VDD}
N 420 -800 430 -800 {lab=VDD}
N 470 -880 470 -830 {lab=VDD}
N 430 -880 470 -880 {lab=VDD}
N 760 -590 780 -590 {lab=EN}
N 820 -660 820 -620 {lab=VDD}
N 820 -590 830 -590 {lab=VDD}
N 830 -660 830 -590 {lab=VDD}
N 820 -660 830 -660 {lab=VDD}
N 820 -560 820 -550 {lab=vpref}
N 570 -530 820 -530 {lab=vpref}
N 820 -550 820 -530 {lab=vpref}
N 670 -660 820 -660 {lab=VDD}
N 110 -560 110 -500 {lab=vpref}
N 110 -440 110 -410 {lab=ibias}
N 150 -470 190 -470 {lab=ENB}
N 100 -470 110 -470 {lab=VDD}
N 100 -590 100 -470 {lab=VDD}
N 90 -210 100 -210 {lab=VSS}
N 90 -210 90 -150 {lab=VSS}
N 90 -150 100 -150 {lab=VSS}
N 100 -180 100 -150 {lab=VSS}
N 100 -150 220 -150 {lab=VSS}
N 140 -210 170 -210 {lab=ENB}
N 100 -300 100 -240 {lab=vnload}
N 100 -300 260 -300 {lab=vnload}
N 70 -150 90 -150 {lab=VSS}
N 830 -210 840 -210 {lab=VSS}
N 840 -210 840 -150 {lab=VSS}
N 830 -150 840 -150 {lab=VSS}
N 830 -180 830 -150 {lab=VSS}
N 760 -210 790 -210 {lab=ENB}
N 670 -150 830 -150 {lab=VSS}
N 540 -280 830 -280 {lab=vout_1st}
N 830 -280 830 -240 {lab=vout_1st}
C {ipin.sym} 200 -400 0 0 {name=p2 lab=vn
}
C {ipin.sym} 490 -400 2 0 {name=p3 lab=vp}
C {lab_pin.sym} 300 -400 0 1 {name=p4 sig_type=std_logic lab=VDD}
C {lab_pin.sym} 400 -400 0 0 {name=p5 sig_type=std_logic lab=VDD}
C {opin.sym} 800 -400 0 0 {name=p7 lab=vout}
C {ipin.sym} 110 -410 3 0 {name=p8 lab=ibias
}
C {iopin.sym} 80 -660 2 0 {name=p9 lab=VDD}
C {iopin.sym} 70 -150 2 0 {name=p10 lab=VSS}
C {title.sym} 170 -30 0 0 {name=l1 author="Luighi Viton"}
C {symbols/cap_nmos_03v3_b.sym} 540 -330 0 0 {name=CM
W=15u
L=15u
model=cap_nmos_03v3_b
spiceprefix=X
m=1}
C {symbols/pfet_05v0.sym} 130 -590 0 1 {name=Mpref0
L=1u
W=0.5u
nf=1
m=2
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_05v0
spiceprefix=X
}
C {symbols/nfet_05v0.sym} 280 -210 0 1 {name=Mnload0
L=2u
W=0.5u
nf=1
m=2
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_05v0
spiceprefix=X
}
C {symbols/pfet_05v0.sym} 330 -590 0 0 {name=Mpref1
L=1u
W=0.5u
nf=1
m=2
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_05v0
spiceprefix=X
}
C {symbols/pfet_05v0.sym} 640 -590 0 0 {name=Mpref2
L=1u
W=0.5u
nf=1
m=8
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_05v0
spiceprefix=X
}
C {symbols/pfet_05v0.sym} 240 -400 0 0 {name=Mpdiff0
L=1u
W=2u
nf=1
m=2
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_05v0
spiceprefix=X
}
C {symbols/pfet_05v0.sym} 460 -400 0 1 {name=Mpdiff1
L=1u
W=2u
nf=1
m=2
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_05v0
spiceprefix=X
}
C {symbols/nfet_05v0.sym} 420 -210 0 0 {name=Mnload1
L=2u
W=0.5u
nf=1
m=2
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_05v0
spiceprefix=X
}
C {symbols/nfet_05v0.sym} 640 -210 0 0 {name=Mncs
L=2u
W=0.5u
nf=1
m=16
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_05v0
spiceprefix=X
}
C {symbols/pfet_05v0.sym} 250 -830 0 1 {name=Mpref_dumm_L[3:0]
L=1u
W=0.5u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_05v0
spiceprefix=X
}
C {lab_wire.sym} 260 -880 0 0 {name=p48 sig_type=std_logic lab=VDD}
C {symbols/pfet_05v0.sym} 450 -830 0 1 {name=Mpref_dumm_TP[23:0]
L=1u
W=0.5u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_05v0
spiceprefix=X
}
C {lab_wire.sym} 460 -880 0 0 {name=Mpref2[13:0] sig_type=std_logic lab=VDD
L=0.6u}
C {lab_wire.sym} 310 -480 0 0 {name=p1 sig_type=std_logic lab=vptail}
C {lab_wire.sym} 260 -270 0 0 {name=p6 sig_type=std_logic lab=vnload}
C {lab_wire.sym} 490 -280 0 0 {name=p11 sig_type=std_logic lab=vout_1st}
C {ipin.sym} 30 -440 0 0 {name=p12 lab=EN}
C {ipin.sym} 30 -380 0 0 {name=p13 lab=ENB}
C {symbols/pfet_05v0.sym} 800 -590 0 0 {name=Mpenpref
L=0.5u
W=1u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_05v0
spiceprefix=X
}
C {lab_wire.sym} 760 -590 0 0 {name=p14 sig_type=std_logic lab=EN}
C {lab_wire.sym} 510 -530 0 0 {name=p16 sig_type=std_logic lab=vpref}
C {symbols/pfet_05v0.sym} 130 -470 0 1 {name=Mpenbias
L=0.5u
W=1u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_05v0
spiceprefix=X
}
C {lab_wire.sym} 190 -470 0 1 {name=p15 sig_type=std_logic lab=ENB}
C {symbols/nfet_05v0.sym} 120 -210 0 1 {name=Mnennref
L=0.6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_05v0
spiceprefix=X
}
C {lab_wire.sym} 150 -210 0 1 {name=p17 sig_type=std_logic lab=ENB}
C {symbols/nfet_05v0.sym} 810 -210 0 0 {name=Mnennref1
L=0.6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_05v0
spiceprefix=X
}
C {lab_wire.sym} 780 -210 0 0 {name=p18 sig_type=std_logic lab=ENB}
C {iopin.sym} 70 -110 2 0 {name=p19 lab=SUB}
C {noconn.sym} 70 -110 2 0 {name=l2}
